    ...
    <script>
        function login() {
            ...
            const instance = axios.create({
                baseURL: "{{ route("auth.authenticate") }}",
                headers: {
            ...
            instance.post('', {
                ...
                const accessToken = response.data.new_tokens[0];
                const token = accessToken.token;
                const expires = new Date(accessToken.claims.exp.date).toUTCString();

                document.cookie = `token=${token}; path=/; SameSite=Strict; expires=${expires}`;
                console.log('Cookie was set successfully.');
                console.log('document.cookie output: ' + document.cookie);
